...
fn runServer(allocator: std.mem.Allocator, session_name: []const u8) !void {
    ...
    // Add stdin to epoll
    var stdin_event = std.os.linux.epoll_event{
        .events = std.os.linux.EPOLL.IN,
        .data = .{ .fd = posix.STDIN_FILENO },
    };
    try posix.epoll_ctl(epoll_fd, std.os.linux.EPOLL.CTL_ADD, posix.STDIN_FILENO, &stdin_event);

    // Add PTY master to epoll
    if (pane.getFd()) |pty_fd| {
        var pty_event = std.os.linux.epoll_event{
            .events = std.os.linux.EPOLL.IN,
            .data = .{ .fd = pty_fd },
        };
        try posix.epoll_ctl(epoll_fd, std.os.linux.EPOLL.CTL_ADD, pty_fd, &pty_event);
    }
    ...
    while (running) {
        const n_events = posix.epoll_wait(epoll_fd, &events, 100); // 100ms timeout
        ...
        // Write output
        _ = posix.write(posix.STDOUT_FILENO, renderer.getOutput()) catch {};
