...
pub const Endpoint = struct {
    ...
    pub fn toSockAddr(self: Endpoint) posix.sockaddr.in {
        return .{
            .family = posix.AF.INET,
            .port = std.mem.nativeToBig(u16, self.port),
            .addr = @bitCast(self.addr),
            .zero = [_]u8{0} ** 8,
        };
    }
};
...
pub const Transport = struct {
    ...
    pub fn bind(self: *Self, port: u16) !void {
        ...
        try posix.bind(self.socket, @ptrCast(&addr), @sizeOf(posix.sockaddr.in6));
    ...
    fn sendRaw(self: *Self, data: []const u8) !void {
        if (self.remote_endpoint) |ep| {
            ...
            _ = posix.sendto(
                ...
                error.WouldBlock => {}, // Non-blocking, ignore
                else => return err,
            };
        }
    }
    ...
    fn processIncoming(self: *Self) !void {
        ...
        while (true) {
            ...
            const header = PacketHeader.deserialize(buf[0..HEADER_SIZE]);

            // Verify checksum
            const expected_checksum = computeChecksum(buf[0..HEADER_SIZE], buf[HEADER_SIZE..len]);
            ...
            if (header.flags.data and header.length > 0) {
                const payload = try self.allocator.dupe(u8, buf[HEADER_SIZE..len]);
...
pub const HolePuncher = struct {
    ...
    pub fn punch(self: *HolePuncher) !void {
        ...
        while (attempts < self.max_attempts) : (attempts += 1) {
            ...
            std.time.sleep(self.punch_interval_ms * std.time.ns_per_ms);
...
test "transport initialization" {
    ...
    try std.testing.expectEqual(Transport.State.closed, transport.state);
}
