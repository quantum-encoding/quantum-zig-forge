...
pub const ProxyServer = struct {
    ...
    pub fn run(self: *ProxyServer) !void {
        ...
        while (self.running.load(.acquire)) {
            ...
            const client = std.posix.accept(socket, @ptrCast(&client_addr), &addr_len, 0) catch |err| {
    ...
    fn handleConnection(self: *ProxyServer, client_socket: std.posix.socket_t) !void {
        ...
        // Read request
        const n = try std.posix.recv(client_socket, &request_buf, 0);
        if (n == 0) return;
        ...
        const request = parser_inst.parseRequest() catch {
            ...
            _ = std.posix.send(client_socket, bad_request, 0) catch {};
            return;
        };
        ...
        const target = self.router_instance.getTarget(&request) orelse {
            ...
            _ = std.posix.send(client_socket, not_found, 0) catch {};
            return;
        };
        ...
        // Send response
        _ = try std.posix.send(client_socket, response_buf[0..response_len], 0);
    ...
    fn proxyToBackend(self: *ProxyServer, pool: *BackendPool, request: *const Request, response_buf: []u8) !usize {
        ...
        _ = std.posix.send(conn.socket, builder.message(), 0) catch {
            be.markFailure();
            return buildErrorResponse(response_buf, 502, "Bad Gateway");
        };

        // Read response
        const resp_len = std.posix.recv(conn.socket, response_buf, 0) catch {
            be.markFailure();
            return buildErrorResponse(response_buf, 502, "Bad Gateway");
        };
...
fn parseListenAddr(addr_str: []const u8, port: u16) !std.posix.sockaddr.in {
