...
pub const TestLibrary = struct {
    ...
    pub fn load(path: []const u8) !TestLibrary {
        ...
        lib.init_fn = @ptrCast(std.c.dlsym(handle, INIT_SYMBOL));
        lib.execute_fn = @ptrCast(std.c.dlsym(handle, EXECUTE_SYMBOL));
        lib.cleanup_fn = @ptrCast(std.c.dlsym(handle, CLEANUP_SYMBOL));
