...
pub const FlateDecode = struct {
    ...
    pub fn decode(allocator: std.mem.Allocator, compressed: []const u8) ![]u8 {
        ...
        // Initial output buffer (estimate 3x expansion)
        var output_size: usize = compressed.len * 3;
        ...
        // Initialize for raw deflate or zlib
            ...
            switch (ret) {
                c.Z_OK => {
                    if (stream.avail_out == 0) {
                        ...
                        const old_len = output.len;
                        output = try allocator.realloc(output, old_len * 2);
                        stream.next_out = output.ptr + old_len;
                        stream.avail_out = @intCast(old_len);
                    }
                },
                c.Z_STREAM_END => {
                    // Success
                    const final_size = output.len - stream.avail_out;
                    return allocator.realloc(output, final_size) catch output[0..final_size];
                },
                c.Z_DATA_ERROR => {
                ...
                else => return error.ZlibDecompressFailed,
            }
        }
    }

    /// Decode raw deflate (no zlib header)
    fn decodeRaw(allocator: std.mem.Allocator, compressed: []const u8) ![]u8 {
        ...
        var ret = c.inflateInit2(&stream, -15);
        if (ret != c.Z_OK) {
            return error.ZlibInitFailed;
        }
        defer _ = c.inflateEnd(&stream);
