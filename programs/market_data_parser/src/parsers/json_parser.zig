...
pub const Parser = struct {
    ...
    pub fn findValue(self: *Parser, key: []const u8) ?[]const u8 {
        // Search pattern: "key":"value" or "key":value
        // We need to find the key, then extract the value
        ...
        while (i < self.buffer.len) {
            ...
            if (std.mem.eql(u8, found_key, key)) {
                ...
                while (i < self.buffer.len and (self.buffer[i] == ' ' or self.buffer[i] == ':')) {
                    i += 1;
                }
                ...
                const value_start = i;
                const first_char = self.buffer[i];
                ...
                if (first_char == '"') {
                    ...
                    self.pos = i + 1;
                    return self.buffer[str_start..i];

                } else if (first_char == '{' or first_char == '[') {
                    // Object or array value - skip for now
                    // TODO: Implement nested structure skipping
                    break;
                    ...
                    self.pos = i;
                    return self.buffer[value_start..i];
                }
            }
