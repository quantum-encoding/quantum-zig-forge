...
fn parseArgs(allocator: std.mem.Allocator) !Args {
    ...
    while (arg_iter.next()) |arg| {
        if (std.mem.eql(u8, arg, "-h") or std.mem.eql(u8, arg, "--help")) {
            ...
            args.config_file = arg_iter.next();
        } else if (std.mem.eql(u8, arg, "-z") or std.mem.eql(u8, arg, "--zone")) {
            if (arg_iter.next()) |zone_file| {
                try args.zone_files.append(allocator, zone_file);
            }
        } else if (std.mem.eql(u8, arg, "-p") or std.mem.eql(u8, arg, "--port")) {
            if (arg_iter.next()) |port_str| {
                args.port = std.fmt.parseInt(u16, port_str, 10) catch 53;
            }
        } else if (std.mem.eql(u8, arg, "-b") or std.mem.eql(u8, arg, "--bind")) {
            if (arg_iter.next()) |addr_str| {
                args.bind_address = parseIPv4(addr_str) catch .{ 0, 0, 0, 0 };
            }
        }
    }
