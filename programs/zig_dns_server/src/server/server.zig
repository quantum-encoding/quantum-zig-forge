    ...
    pub fn handleQuery(self: *QueryHandler, query_data: []const u8, response_buf: []u8) !usize {
        ...
        if (answer_count == 0 and question.qtype != .CNAME) {
            ...
            if (cname_count > 0) {
                ...
                final_count = 1;
                // TODO: Chase CNAME to get actual records
            }
        }
...
pub const Server = struct {
    ...
    pub fn start(self: *Server) !void {
        ...
        errdefer std.posix.close(udp_sock);

        // Allow address reuse
        const enable: i32 = 1;
        try std.posix.setsockopt(udp_sock, std.posix.SOL.SOCKET, std.posix.SO.REUSEADDR, std.mem.asBytes(&enable));

        // Bind UDP
        const addr = parseAddr(self.config.listen_addr, self.config.port) catch return error.InvalidAddress;
        try std.posix.bind(udp_sock, @ptrCast(&addr), @sizeOf(std.posix.sockaddr.in));

        self.udp_socket = udp_sock;

        // Create TCP socket if enabled
        if (self.config.tcp_port > 0) {
            const tcp_sock = try std.posix.socket(std.posix.AF.INET, std.posix.SOCK.STREAM, 0);
            errdefer std.posix.close(tcp_sock);

            try std.posix.setsockopt(tcp_sock, std.posix.SOL.SOCKET, std.posix.SO.REUSEADDR, std.mem.asBytes(&enable));

            const tcp_addr = parseAddr(self.config.listen_addr, self.config.tcp_port) catch return error.InvalidAddress;
            try std.posix.bind(tcp_sock, @ptrCast(&tcp_addr), @sizeOf(std.posix.sockaddr.in));
    ...
    pub fn run(self: *Server) !void {
        ...
        while (self.running.load(.acquire)) {
            ...
            const recv_len = std.posix.recvfrom(udp_sock, &query_buf, 0, &client_addr, &addr_len) catch |err| {
            ...
            // Send response
            _ = std.posix.sendto(udp_sock, final_response, 0, &client_addr, addr_len) catch {
                _ = self.stats.queries_failed.fetchAdd(1, .monotonic);
                continue;
            };
