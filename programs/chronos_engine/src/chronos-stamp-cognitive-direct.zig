...
fn findLatestStateMap() !c_int {
    ...
    while (c.bpf_map_get_next_id(map_id, &next_id) == 0) {
        ...
        defer _ = std.posix.close(@intCast(map_fd));
...
fn getClaudePID() !u32 {
    ...
    while (depth < 20) : (depth += 1) {
        ...
        const bytes_read = file.read(&stat_buf) catch break;
...
pub fn main() !void {
    ...
    std.posix.nanosleep(0, 250_000_000); // 250ms - gives watcher time to capture and write state
