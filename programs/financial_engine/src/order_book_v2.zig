...
pub const PriceLevel = struct {
    ...
    order_count: u32,
    orders: std.ArrayListAligned(*Order, null),
    ...
    pub fn deinit(self: *PriceLevel, allocator: std.mem.Allocator) void {
        self.orders.deinit(allocator);
    }
    
    pub fn addOrder(self: *PriceLevel, allocator: std.mem.Allocator, order: *Order) !void {
        try self.orders.append(allocator, order);
...
pub const OrderBook = struct {
    ...
    symbol: []const u8,
    bids: std.ArrayListAligned(PriceLevel, null),
    asks: std.ArrayListAligned(PriceLevel, null),
    trades: std.ArrayListAligned(Trade, null),
    ...
    pub fn addOrder(self: *Self, side: Side, order_type: OrderType, price: Decimal, quantity: Decimal, client_id: u32) !*Order {
        ...
        order.* = .{
            ...
            .timestamp = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
            .status = .pending,
            .client_id = client_id,
        };
    ...
    fn executeLimitOrder(self: *Self, order: *Order) !void {
        ...
        while (!order.isFilled() and opposite_book.items.len > 0) {
            ...
            if (best_level.orders.items.len == 0) {
                best_level.deinit(self.allocator);
                _ = opposite_book.orderedRemove(0);
            }
        }
    ...
    fn addToBook(self: *Self, order: *Order, book: *std.ArrayListAligned(PriceLevel, null)) !void {
        ...
        if (level_index) |idx| {
            ...
            if (idx < book.items.len and book.items[idx].price.equals(order.price)) {
                ...
                try book.items[idx].addOrder(self.allocator, order);
            } else {
                // Insert new level
                var new_level = PriceLevel.init(self.allocator, order.price);
                try new_level.addOrder(self.allocator, order);
    ...
    pub fn cancelOrder(self: *Self, order_id: u64) bool {
        ...
        for (book.items) |*level| {
            if (level.removeOrder(order_id)) |_| {
                if (level.orders.items.len == 0) {
                    // Remove empty level
