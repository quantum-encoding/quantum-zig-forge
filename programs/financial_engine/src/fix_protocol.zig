...
pub const Session = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, sender: []const u8, target: []const u8) Self {
        return .{
            ...
            .last_heartbeat = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
            .allocator = allocator,
        };
    }
    ...
    pub fn createNewOrder(
        ...
        // Transaction time
        const time_str = try std.fmt.allocPrint(self.allocator, "{d}", .{@as(i64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec))});
        defer self.allocator.free(time_str);
    ...
    pub fn createHeartbeat(self: *Self) ![]const u8 {
        ...
        self.outgoing_seq_num += 1;
        self.last_heartbeat = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec);
        return try builder.finalize();
    }
