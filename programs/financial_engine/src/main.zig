...
pub const TradingEngine = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, initial_balance: Decimal) !Self {
        ...
        return Self{
            ...
            .allocator = allocator,
            .message_count = 0,
            .last_update = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
        };
    }
    ...
    pub fn submitOrder(
        ...
        self.last_update = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec);
