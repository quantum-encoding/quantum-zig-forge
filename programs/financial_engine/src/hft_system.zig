...
pub const HFTSystem = struct {
    ...
    pub const SystemMetrics = struct {
        ...
        pub fn init() SystemMetrics {
            return .{
                ...
                .avg_latency_us = 0,
                .peak_latency_us = 0,
                .start_time = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
            };
        }
    ...
    pub fn getPerformanceReport(self: Self) void {
        const elapsed = @as(i64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec)) - self.metrics.start_time;
...
pub const MarketSimulator = struct {
    ...
    pub fn init(base_price: f64, volatility: f64, spread: f64) Self {
        return .{
            ...
            .spread = spread,
            .time = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
            .sequence = 0,
            .rng = std.Random.DefaultPrng.init(@as(u64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec))),
        };
    }
    ...
    pub fn generateTick(self: *Self, symbol: []const u8) MarketTick {
        ...
        self.sequence += 1;
        self.time = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec);
...
pub fn main() !void {
    ...
    var hft = try HFTSystem.init(allocator, config);
    defer hft.deinit();
    ...
    const start = blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);};
    ...
    for (0..num_ticks) |i| {
        ...
        // Progress update
        if (i % 100 == 0 and i > 0) {
            const elapsed = blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);} - start;
            const rate = @as(f64, @floatFromInt(i * 1000)) / @as(f64, @floatFromInt(elapsed));
            std.debug.print("Processed {d} ticks - Rate: {d:.0} ticks/sec\n", .{ i, rate });
        }
    }
    
    const total_elapsed = blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);} - start;
