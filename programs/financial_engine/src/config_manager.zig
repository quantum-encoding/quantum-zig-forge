...
pub const ConfigManager = struct {
    ...
    pub fn loadFromEnvironment(self: *ConfigManager) !void {
        if (std.process.getEnvVarOwned(self.allocator, "ALPACA_API_KEY")) |key| {
            self.config.api_credentials.api_key = key;
        } else |_| {}

        if (std.process.getEnvVarOwned(self.allocator, "ALPACA_API_SECRET")) |secret| {
            self.config.api_credentials.api_secret = secret;
        } else |_| {}

        if (std.process.getEnvVarOwned(self.allocator, "ALPACA_PAPER_TRADING")) |paper| {
            defer self.allocator.free(paper);
...
pub fn parseArgs(allocator: std.mem.Allocator) !?[]const u8 {
