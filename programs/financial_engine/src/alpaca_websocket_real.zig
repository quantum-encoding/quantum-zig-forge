...
pub const AlpacaWebSocketReal = struct {
    ...
    fn processQuote(self: *Self, obj: json.ObjectMap) !void {
        ...
        if (obj.get("t")) |t| {
            ...
            quote.timestamp = std.fmt.parseInt(i64, ts_str, 10) catch blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);};
        }
    ...
    fn processTrade(self: *Self, obj: json.ObjectMap) !void {
        ...
        if (obj.get("t")) |t| {
            const ts_str = t.string;
            trade.timestamp = std.fmt.parseInt(i64, ts_str, 10) catch blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);};
        }
