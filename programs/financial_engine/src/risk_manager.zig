...
pub const RiskManager = struct {
    ...
    pub fn openPosition(self: *Self, symbol: []const u8, side: @TypeOf(@as(Position, undefined).side), quantity: Decimal, price: Decimal) !void {
        ...
        const position = Position{
            ...
            .stop_loss = null,
            .take_profit = null,
            .opened_at = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
            .updated_at = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec),
        };
    ...
    pub fn updatePrice(self: *Self, symbol: []const u8, price: Decimal) void {
        if (self.positions.getPtr(symbol)) |position| {
            ...
            position.updated_at = @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec);
