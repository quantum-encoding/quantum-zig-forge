...
pub const RealHFTSystem = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: SystemConfig) !Self {
        ...
        var hft = try hft_system.HFTSystem.init(allocator, hft_config);
    ...
    pub fn run(self: *Self, duration_seconds: ?u32) !void {
        ...
        const start_time = @as(i64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec));
        ...
        while (self.is_running.load(.acquire) and @as(i64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec)) < end_time) {
            // Print stats every 10 seconds
            const current_time = @as(i64, @intCast((std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable).sec));
...
pub fn main() !void {
    ...
    const config_file = try config_manager.parseArgs(allocator);
    if (config_file == null) {
