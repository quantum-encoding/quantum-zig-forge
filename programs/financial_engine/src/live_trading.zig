...
pub const LiveTradingSystem = struct {
    ...
    pub fn init(
        ...
        hft_ptr.* = try hft_system.HFTSystem.init(allocator, hft_config);
    ...
    pub fn processLoop(self: *Self) !void {
        ...
        const start_time = blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);};
        ...
        while (self.is_running.load(.acquire)) {
            ...
            if (tick_count % 1000 == 0 and tick_count > 0) {
                const elapsed = blk: {const ts = std.posix.clock_gettime(std.posix.CLOCK.REALTIME) catch unreachable; break :blk @as(i64, @intCast(ts.sec)) * 1000 + @divTrunc(ts.nsec, 1_000_000);} - start_time;
