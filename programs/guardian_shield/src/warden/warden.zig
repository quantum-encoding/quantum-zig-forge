...
var runtime_rules: std.ArrayListUnmanaged(ProtectionRule) = .{};
...
fn writeRuntimeConfig() !void {
    ...
    const file = try std.fs.cwd().createFile(runtime_config_path, .{});
    defer file.close();

    writeToFile(file, "{\n");
    writeToFile(file, "  \"_comment\": \"Runtime protection rules from embedded warden module\",\n");
    writeToFile(file, "  \"runtime_rules\": [\n");

    for (runtime_rules.items, 0..) |rule, i| {
        if (i > 0) writeToFile(file, ",\n");
        writeToFile(file, "    {\n");

        var path_buf: [512]u8 = undefined;
        const path_line = std.fmt.bufPrint(&path_buf, "      \"path\": \"{s}\",\n", .{rule.path}) catch continue;
        writeToFile(file, path_line);
...
fn writeChildConfig(path: []const u8, rules: []const ProtectionRule, inherit: bool) !void {
    const file = try std.fs.cwd().createFile(path, .{});
    defer file.close();

    writeToFile(file, "{\n");
    writeToFile(file, "  \"_comment\": \"Child process protection rules\",\n");
    ...
    writeToFile(file, inherit_line);

    writeToFile(file, "  \"protection\": {\n");
    writeToFile(file, "    \"protected_paths\": [\n");
...
fn signalReload() !void {
    ...
    const file = std.fs.cwd().createFile(reload_flag, .{}) catch return;
    file.close();
}
