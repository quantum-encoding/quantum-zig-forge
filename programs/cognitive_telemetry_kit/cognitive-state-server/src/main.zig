...
fn watchDatabaseChanges(allocator: std.mem.Allocator, cache: *StateCache) !void {
    ...
    while (true) {
        ...
        // Sleep briefly to avoid busy-waiting (Zig 0.16: use posix.nanosleep)
        std.posix.nanosleep(0, 100 * std.time.ns_per_ms);
    }
}

// Zig 0.16 compatibility: Get milliseconds since epoch
...
pub fn main() !void {
    ...
    while (true) {
        ...
        std.posix.nanosleep(0, 10 * std.time.ns_per_ms); // 10ms sleep to avoid busy-waiting
    }
}
