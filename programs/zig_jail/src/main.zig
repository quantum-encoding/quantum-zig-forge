...
const VERSION = "2.0.0";
const SIGSYS = 31; // seccomp violation signal
...
pub fn main() !void {
    ...
    if (pid == 0) {
        ...
        const argv = try allocator.alloc([*:0]const u8, command_args.len);
        ...
        // Note: std.process.execve handles PATH resolution and never returns on success
        return std.process.execve(allocator, command_args, null);
    } else {
        ...
        if (std.os.linux.W.IFEXITED(status)) {
            ...
            std.debug.print("\n[zig-jail] 🛡️  Process terminated by signal {d}", .{signal});
            if (signal == SIGSYS) {
            ...
            std.process.exit(128 + @as(u8, @intCast(signal)));
        }
    }
}
